...
type SomeParent = TNode<"SomeParent", {
  username: string
}>

const SomeParent = nodeType<SomeParent>("SomeParent")
.onInit((node) => {
  usernameCtx.setComputed(() => node.username)
})

type SomeDeepChild = TNode<"SomeDeepChild", {
  someValue: string
}>
const SomeDeepChild = nodeType<SomeDeepChild>("SomeDeepChild")
.actions({
...
.computeds({
  someComputedThatRequiresUsername() {
...
In particular, `apply` is more useful than it may seem, since it allows you to pass volatile (non-property) data to your nodes at construction time (be it through the node type constructor function, `node`, `clone`, etc.).
For example:
...
const M = nodeType<M>("M")
  .onInit((node) => {
    const value = envCtx.get(node)
  })
  .getters({
    value() {
