...
- 📦 An **observable plain object** (for structured data)
- 📋 An **observable array** (for collections)
...
3. 🔄 From rules 1 and 2, a non-primitive node can exist in **only one place in the tree**.
4. 📝 Primitive nodes are always **copied by value**, so they aren't subject to the rules above.
...
> 💡 **Important Detail**: When a plain object is added to a tree, its reference changes as it becomes a node. This is standard behavior in MobX deep observables:
