...
```ts
import { nodeType, objectToMapTransform, timestampToDateTransform, dateToTimestampTransform } from 'mobx-bonsai';
...
const instance = TMyType({ record: { a: 1, b: 2 }, array: ["a", "b"], timestamp: 1633072800000 });

const map = TMyType.getMap(instance); // Map<string, number>

const set = TMyType.getSet(instance); // Set<string>

const date = TMyType.getTimestampDate(instance); // ImmutableDate
