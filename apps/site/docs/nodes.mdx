...
This function is similar to `onDeepChange`, but the listener is called during MobX's intercept phase, **before** the change is applied to the node. This is useful when you need to capture state right before modifications occur:
...
- Receives `IObjectWillChange`, `IArrayWillChange`, or `IArrayWillSplice` change objects
