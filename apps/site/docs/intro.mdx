...
- **🧭 Tree Navigation**: Easily traverse your state tree with powerful helpers like `getParent`, `getRoot`, and `findChildren`.
...
- **👥 Real-time Collaboration Ready**: Built-in two-way binding with Y.js (via the `mobx-bonsai-yjs` package) for effortless real-time collaboration features.
...
For Y.js integration, also install the `mobx-bonsai-yjs` package:
